...
def validate_email(email):
    """
    Validate an email address against a common practical pattern.
...
This principle goes by several names depending on the audience. Philosophers and ethicists call it **human agency**: the capacity to choose, weigh, and own the outcome. Practitioners and team leads call it **human oversight**: the practice of reviewing AI output before it ships. AI policy and system design call it **human-in-the-loop** (HIL): the structural requirement that a person remains an active participant in decisions a system could otherwise automate. **Intentional prompting** is the methodology that operationalises all three. Agency is the principle, oversight is the practice, HIL is the system-design view, and intentional prompting is the workflow that keeps them in place during development. Four vocabularies, one stance.
...
The six-step programming methodology (which we'll explore in depth in Part III, Methodology) provides a structured process that works with or without AI assistance:
...
Hands-on evaluation has documented significant variation in LLM performance across multiple runs: the same prompt can produce answers of materially different quality, and the gap between the best and worst of several attempts can be larger than the difference between competing models. Two sources of this variability stand out:
